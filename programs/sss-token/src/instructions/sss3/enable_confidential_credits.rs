...
use crate::{
    errors::SssError,
    events::ConfidentialCreditsEnabled,
    state::StablecoinState,
};

/// Enable confidential credits for a token account.
/// Allows the account to receive confidential transfers.
/// Must be called by the token account owner.
#[derive(Accounts)]
pub struct EnableConfidentialCredits<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<EnableConfidentialCredits>) -> Result<()> {
    let token_account_key = ctx.accounts.token_account.key();
    let owner_key = ctx.accounts.owner.key();
    let token_program_id = ctx.accounts.token_program.key();

    let ix = confidential_transfer::instruction::enable_confidential_credits(
        &token_program_id,
        &token_account_key,
        &owner_key,
        &[], // no multisig signers
