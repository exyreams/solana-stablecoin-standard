...
describe("sss-oracle", () => {
  ...
  describe("initialize_oracle", () => {
    it("creates oracle config with correct state", async () => {
      ...
      const config = await program.account.oracleConfig.fetch(
        ctx.oracleConfig
      );
    ...
    it("derives correct PDA from mint key", async () => {
      const mint = Keypair.generate();
      const [expected] = findOracleConfigPda(
        mint.publicKey,
        program.programId
      );
  ...
  describe("update_oracle_config", () => {
    ...
    it("can pause and unpause", async () => {
      ...
      let config = await program.account.oracleConfig.fetch(
        ctx.oracleConfig
      );
  ...
  describe("transfer_oracle_authority", () => {
    ...
    it("completes two-step authority transfer", async () => {
      ...
      assert.ok(
        ...
        new PublicKey(config.pendingAuthority).equals(
  ...
  describe("aggregate", () => {
    ...
    it("aggregates three feeds with median", async () => {
      ...
      await crankFeed(ctx, feed1.feedPda, toFixedPoint(1.10));
