...
describe("integration tests", () => {
  ...
  const oracleProgram = anchor.workspace
    .SssOracle as Program<SssOracle>;
  ...
  describe("full stablecoin lifecycle", () => {
    ...
    it("step 2: add feeds and crank prices", async () => {
      ...
      assert.equal(
        f0.lastPrice.toString(),
        toFixedPoint(1.085).toString()
      );
      ...
      assert.equal(
        f1.lastPrice.toString(),
        toFixedPoint(1.087).toString()
      );
    });
  ...
  describe("multi-minter scenario", () => {
    it("independent minters with separate quotas", async () => {
      ...
      const quotaA = await addMinter(
        tokenCtx,
        minterA.publicKey,
        1_000
      );
      const quotaB = await addMinter(
        tokenCtx,
        minterB.publicKey,
        500
      );
