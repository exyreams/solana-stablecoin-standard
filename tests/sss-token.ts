...
describe("sss-token", () => {
  ...
  describe("initialize", () => {
    it("creates SSS-1 stablecoin with correct state", async () => {
      ...
      const roles = await program.account.rolesConfig.fetch(
        ctx.rolesConfig
      );
  ...
  describe("minter management", () => {
    ...
    describe("add_minter", () => {
      it("creates minter quota PDA", async () => {
        ...
        const q = await program.account.minterQuota.fetch(
          minterQuotaPda
        );
  ...
  describe("transfer_authority", () => {
    ...
    it("completes two-step transfer", async () => {
      ...
      let roles = await program.account.rolesConfig.fetch(
        ctx.rolesConfig
      );
      assert.ok(
  ...
  describe("blacklist (SSS-2)", () => {
    ...
    it("adds address to blacklist", async () => {
      ...
      const entry = await program.account.blacklistEntry.fetch(
        blacklistEntry
      );
