...
app.post("/metadata/metaplex", async (c) => {
  const { name, symbol, uri, mintKeypairPath } = await c.req.json();

  if (!name || !symbol || !uri) {
    c.status(400);
    return c.json({ error: "Missing required fields: name, symbol, uri" });
  }

  if (!mintKeypairPath) {
    c.status(400);
    return c.json({
      error: "mintKeypairPath required. Metaplex requires mint keypair to sign."
    });
  }
  ...
  try {
    ...
    // Load mint keypair from provided path
    const fs = await import("fs");
    const mintSecretKey = JSON.parse(fs.readFileSync(mintKeypairPath, "utf-8"));
    const { Keypair } = await import("@solana/web3.js");
    const mintKeypair = Keypair.fromSecretKey(Uint8Array.from(mintSecretKey));

    // Verify mint keypair matches
    if (!mintKeypair.publicKey.equals(s.mint)) {
      c.status(400);
      return c.json({
        error: "Mint keypair does not match stablecoin mint address"
      });
    }

    const metadataPda = await s.initializeMetaplexMetadata({
      name,
      symbol,
      uri,
    ...
    await db.insert(auditLogs).values({
      action: "INITIALIZE_METAPLEX_METADATA",
      address: metadataPda.toBase58(),
      reason: `Metaplex metadata: ${name} (${symbol})`,
      signature: metadataPda.toBase58(), // PDA address as reference
    });

    log.info({ metadataPda: metadataPda.toBase58(), name, symbol }, "Metaplex metadata initialized");
    return c.json({
      success: true,
      metadataPda: metadataPda.toBase58(),
      message: "Token will now display in wallets (Phantom, Solflare, etc.)"
    });
  } catch (err: any) {
    log.error({ err: err.message }, "Failed to create Metaplex metadata");
    c.status(500);
    return c.json({ error: err.message });
  }
});
