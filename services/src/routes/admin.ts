...
app.post("/metadata/initialize", async (c) => {
	try {
		const s = await getStable();
		const sig = await s.initializeMetadata();

		await db.insert(auditLogs).values({
			action: "INITIALIZE_METADATA",
			address: s.mint.toBase58(),
			reason: "Token-2022 Metadata creation",
			signature: sig,
		});
		log.info({ sig }, "Metadata initialized");
		return c.json({ success: true, signature: sig });
	} catch (err: any) {
		log.error({ err: err.message }, "Failed to initialize metadata");
		c.status(500);
		return c.json({ error: err.message });
	}
});
...
app.post("/metadata/metaplex", async (c) => {
	try {
		...
		return c.json({
			error:
				"Metaplex metadata support temporarily disabled. Token-2022 on-mint metadata is available via /metadata/initialize",
		});
	} catch (err: any) {
