...
app.post("/", async (c) => {
  try {
    ...
    log.info(`Transfer hook program ID: ${transferHookProgramId?.toBase58() || 'none'}`);
    log.info(`Preset: ${preset}, Extensions: ${JSON.stringify(extensions)}`);

    // Create the stablecoin
    try {
      const { stablecoin, signature: creationSignature } = await SolanaStablecoin.create(connection, {
      ...
      if (preset === "sss2" && stablecoin.compliance) {
        ...
        while (retries < maxRetries) {
          try {
            ...
            const mintInfo = await connection.getAccountInfo(mintKeypair.publicKey, "finalized");
            if (!mintInfo) {
              log.warn(`Attempt ${retries + 1}: Mint account not fully finalized on RPC yet...`);
              throw new Error("Mint account not finalized");
            }

            // Verify the mint account properties
            if (!mintInfo.owner.equals(TOKEN_2022_PROGRAM_ID)) {
              throw new Error(`Owner mismatch: ${mintInfo.owner.toBase58()}, expected ${TOKEN_2022_PROGRAM_ID.toBase58()}`);
            }

            if (mintInfo.data.length < 82) {
              throw new Error(`Invalid mint data size: ${mintInfo.data.length}`);
            }

            log.info(`Attempt ${retries + 1}: Mint finalized (${mintInfo.data.length} bytes). Waiting 2s for RPC state consistency...`);
            ...
            if (hookError.logs) {
              log.error(`Hook Init Simulation Failure Logs: ${JSON.stringify(hookError.logs, null, 2)}`);
            }

            log.warn(`Transfer hook initialization attempt ${retries} failed: ${hookError.message}`);

            if (retries >= maxRetries) {
              log.error("Transfer hook initialization failed after all retries. The mint was created but the hook requires manual initialization.");
              break;
            }
          }
        }
      ...
      if (sdkError.logs) {
        log.error(`Full Transaction Logs: ${JSON.stringify(sdkError.logs, null, 2)}`);
      }

      // Check for specific transfer hook error
      if (sdkError.message?.includes("Transfer hook program ID not configured")) {
        c.status(400);
        return c.json({
