...
export class OracleModule {
  ...
  constructor(
    ...
    const programId = oracleProgramId ?? new PublicKey(
      ORACLE_IDL.address ?? ORACLE_IDL.metadata?.address
    );

    // Create a proper Anchor provider
    const { AnchorProvider, Wallet } = require('@coral-xyz/anchor');
    const wallet = new Wallet(authority);
    const provider: typeof AnchorProvider = new AnchorProvider(connection, wallet, { commitment: 'confirmed' });
  ...
  async initialize(options: OracleInitializeOptions): Promise<string> {
    const [oracleConfigPda] = deriveOracleConfig(this.mint, this.oracleProgram.programId);
  ...
  async updateConfig(options: OracleUpdateConfigOptions): Promise<string> {
    ...
    const params = {
      maxStalenessSeconds: options.maxStalenessSeconds !== undefined
  ...
  async getStatus(): Promise<OracleStatus> {
    const [oracleConfigPda] = deriveOracleConfig(this.mint, this.oracleProgram.programId);
    const config = await (this.oracleProgram.account as any).oracleConfig.fetch(oracleConfigPda);
    ...
    return {
      ...
      lastAggregatedConfidence: BigInt(config.lastAggregatedConfidence.toString()),
  ...
  async getFeeds(): Promise<PriceFeedInfo[]> {
    ...
    const accounts = await (this.oracleProgram.account as any).priceFeedEntry.all([
      {
        memcmp: {
  ...
  async getFeed(feedIndex: number): Promise<PriceFeedInfo | null> {
    ...
    try {
      const feed = await (this.oracleProgram.account as any).priceFeedEntry.fetch(feedPda);
      return {
