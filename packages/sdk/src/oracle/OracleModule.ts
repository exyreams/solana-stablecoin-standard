...
export class OracleModule {
	...
	constructor(
		...
		const programId =
			oracleProgramId ??
			new PublicKey(ORACLE_IDL.address ?? ORACLE_IDL.metadata?.address);

		// Create a proper Anchor provider
		const { AnchorProvider, Wallet } = require("@coral-xyz/anchor");
		const wallet = new Wallet(authority);
		const provider: typeof AnchorProvider = new AnchorProvider(
			connection,
			wallet,
			{ commitment: "confirmed" },
		);

		this.oracleProgram = new Program(ORACLE_IDL as any, programId, provider);
	}
