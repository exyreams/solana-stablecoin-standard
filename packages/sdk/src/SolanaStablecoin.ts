...
// Path assumes monorepo: packages/sdk/ → ../../target/idl/
...
export class SolanaStablecoin {
  ...
  static async create(
    ...
    const enablePermanentDelegate =
      options.enablePermanentDelegate ?? ext.permanentDelegate ?? preset.enablePermanentDelegate;
    const enableTransferHook =
      options.enableTransferHook ?? ext.transferHook ?? preset.enableTransferHook;
    const defaultAccountFrozen =
      options.defaultAccountFrozen ?? ext.defaultAccountFrozen ?? preset.defaultAccountFrozen;
    ...
    const confidentialTransferAutoApprove =
      options.confidentialTransferAutoApprove ?? preset.confidentialTransferAutoApprove;
  ...
  async addMinter(minter: PublicKey, quota: bigint): Promise<string> {
    const [statePda] = deriveStablecoinState(this.mint, this.program.programId);
    const [rolesPda] = deriveRolesConfig(this.mint, this.program.programId);
    const [quotaPda] = deriveMinterQuota(this.mint, minter, this.program.programId);
  ...
  async getRoles(): Promise<RolesStatus> {
    const [rolesPda] = deriveRolesConfig(this.mint, this.program.programId);
    const roles = await (this.program.account as any).rolesConfig.fetch(rolesPda);
