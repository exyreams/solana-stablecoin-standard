...
export function mintersCommand(): Command {
  ...
  cmd
    ...
    .action(async (minterAddress, opts, cmd) => {
      ...
      try {
        ...
        const sig = await stable.addMinter(new PublicKey(minterAddress), rawQuota);
        spinner.succeed(`Minter added: ${minterAddress}`);
        success(`Quota: ${opts.quota === '0' ? 'Unlimited' : opts.quota + ' ' + status.symbol}`);
        printTxLink(sig);
      } catch (err: any) {
  ...
  cmd
    ...
    .description('Update a minter\'s quota, active status, or reset minted counter')
