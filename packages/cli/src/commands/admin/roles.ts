  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        if (opts.blacklister) update.blacklister = new PublicKey(opts.blacklister);
        if (opts.seizer) update.seizer = new PublicKey(opts.seizer);

        if (Object.keys(update).length === 0) {
          spinner.fail('No roles specified. Use --burner, --pauser, --blacklister, or --seizer');
          process.exit(1);
        }
  ...
  cmd
    ...
    .action(async (newMaster, opts, cmd) => {
      ...
      try {
        ...
        success('The new master must call "sss-token roles accept" to complete the transfer');
        printTxLink(sig);
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        spinner.succeed('Authority transfer accepted — you are now the master authority');
        printTxLink(sig);
      } catch (err: any) {
