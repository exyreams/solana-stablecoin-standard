...
export function seizeCommand(): Command {
  ...
  cmd
    .description('Seize tokens from an account to treasury via permanent delegate (SSS-2 only)')
    ...
    .action(async (fromTokenAccount, opts, cmd) => {
      ...
      try {
        ...
        spinner.succeed(`Seized ${opts.amount} ${status.symbol} from ${fromTokenAccount}`);
        printTxLink(sig);
      } catch (err: any) {
