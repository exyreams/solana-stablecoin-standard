...
export function hookCommand(): Command {
  ...
  cmd
    .command('init')
    .description('Initialize the ExtraAccountMetaList for the transfer hook (call once after mint creation)')
    .action(async (opts, cmd) => {
      const globals = cmd.parent!.parent!.opts();
      const spinner = ora('Initializing transfer hook ExtraAccountMetaList...').start();
      try {
        ...
        success('ExtraAccountMetaList PDA created — blacklist checks are now active on all transfers');
        printTxLink(sig);
      } catch (err: any) {
