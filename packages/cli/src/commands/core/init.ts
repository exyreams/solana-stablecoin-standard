...
export function initCommand(): Command {
  ...
  cmd
    ...
    .option('--preset <preset>', 'Preset to use: sss-1, sss-2, or sss-3', 'sss-1')
    ...
    .option('--transfer-hook-program <programId>', 'Transfer hook program ID (SSS-2)')
    .option('--auto-approve', 'Auto-approve new accounts for confidential transfers (SSS-3)', false)
    .option('--auditor-elgamal-pubkey <pubkey>', 'Auditor ElGamal public key hex (SSS-3)')
      ...
      try {
        ...
        if (opts.custom) {
          ...
          const stable = await SolanaStablecoin.create(connection, {
            ...
            authority,
            enablePermanentDelegate: customConfig.enable_permanent_delegate ?? false,
            ...
            transferHookProgramId: customConfig.transfer_hook_program_id
              ...
              : undefined,
            enableConfidentialTransfers: customConfig.enable_confidential_transfers ?? false,
            confidentialTransferAutoApprove: customConfig.confidential_transfer_auto_approve ?? false,
          ...
          printInitSummary(stable.mint, customConfig.name ?? opts.name, opts.symbol, 'custom');
          return;
        }
        ...
        if (preset === Presets.SSS_2) {
          ...
          createConfig.transferHookProgramId = new PublicKey(opts.transferHookProgram);
        }

        // SSS-3: optional params
        if (preset === Presets.SSS_3) {
          createConfig.confidentialTransferAutoApprove = opts.autoApprove ?? false;
          if (opts.auditorElgamalPubkey) {
            createConfig.auditorElGamalPubkey = Buffer.from(opts.auditorElgamalPubkey, 'hex');
          }
        }
...
function printInitSummary(mint: PublicKey, name: string, symbol: string, preset: string) {
  printTable(
    ['Field', 'Value'],
    [
      ['Mint', mint.toBase58()],
