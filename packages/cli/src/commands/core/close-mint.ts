...
import { success, printTxLink, warn, error } from '../../utils/display';

export function closeMintCommand(): Command {
  const cmd = new Command('close-mint');
  cmd
    .description('Permanently close the mint and reclaim all rent (supply must be zero, irreversible)')
    .option('--force', 'Skip confirmation prompt', false)
    .action(async (opts, cmd) => {
      const globals = cmd.parent!.opts();

      if (!opts.force) {
        warn('⚠️  This is IRREVERSIBLE. The mint, state, and roles will be permanently destroyed.');
        warn('   All MinterQuota PDAs should be removed first via "minters remove".');
