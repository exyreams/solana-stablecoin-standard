...
import ora from 'ora';
import { SolanaStablecoin, AggregationMethod, FeedType } from '@stbr/sss-token-sdk';
import { loadKeypair } from '../../utils/keypair';
import { success, printTxLink, error, printTable, info } from '../../utils/display';
...
export function oracleCommand(): Command {
  ...
  cmd
    ...
    .option('--method <method>', 'Aggregation method: median, mean, weighted', 'median')
    .option('--mint-premium <bps>', 'Mint premium in bps', '0')
    .option('--redeem-discount <bps>', 'Redeem discount in bps', '0')
    .option('--circuit-breaker <bps>', 'Max single-crank price change in bps (0 = disabled)', '500')
    .option('--deviation <bps>', 'Max deviation threshold in bps (0 = disabled)', '0')
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const methodLabel = ['Median', 'Mean', 'Weighted Mean'][status.aggregationMethod];
        ...
        printTable(
          ...
          [
            ...
            ['Pending Authority', status.pendingAuthority ? status.pendingAuthority.toBase58() : '(none)'],
            ...
            ['Last Aggregated Price', status.lastAggregatedPrice ? String(status.lastAggregatedPrice) : '(none)'],
            ['Last Aggregation Time', status.lastAggregatedTimestamp ? new Date(Number(status.lastAggregatedTimestamp) * 1000).toISOString() : '(none)'],
          ],
        );
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const update: any = {};
        if (opts.staleness) update.maxStalenessSeconds = parseInt(opts.staleness);
        if (opts.confidence) update.maxConfidenceIntervalBps = parseInt(opts.confidence);
        if (opts.method) {
          switch (opts.method.toLowerCase()) {
            case 'mean':
        ...
        if (opts.mintPremium) update.mintPremiumBps = parseInt(opts.mintPremium);
        if (opts.redeemDiscount) update.redeemDiscountBps = parseInt(opts.redeemDiscount);
        if (opts.circuitBreaker) update.maxPriceChangeBps = parseInt(opts.circuitBreaker);
        if (opts.deviation) update.deviationThresholdBps = parseInt(opts.deviation);
  ...
  cmd
    ...
    .requiredOption('--type <type>', 'Feed type: switchboard, pyth, chainlink, manual, api')
    .requiredOption('--address <pubkey>', 'Feed address (PublicKey for on-chain feeds)')
    .option('--label <label>', 'Feed label', '')
    .option('--weight <n>', 'Feed weight for weighted mean aggregation', '1')
    .option('--staleness <seconds>', 'Per-feed staleness override (0 = use global)', '0')
    .action(async (opts, cmd) => {
      const globals = cmd.parent!.parent!.opts();
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const connection = new Connection(globals.url, 'confirmed');
        const stable = await SolanaStablecoin.load(connection, new PublicKey(globals.mint), authority);
        const sig = await stable.oracle.setManualPrice(BigInt(opts.price), opts.active);
        spinner.succeed(`Manual price set: ${opts.price} (${opts.active ? 'active' : 'inactive'})`);
        printTxLink(sig);
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const price = opts.side.toLowerCase() === 'redeem'
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        const feedTypeLabels = ['Switchboard', 'Pyth', 'Chainlink', 'Manual', 'API'];
        const rows = feeds.map((f: any) => [
          ...
          f.lastUpdateTimestamp ? new Date(Number(f.lastUpdateTimestamp) * 1000).toISOString() : '(none)',
        ]);

        printTable(['Index', 'Type', 'Address', 'Label', 'Enabled', 'Weight', 'Last Price', 'Last Update'], rows);
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (newAuthority, opts, cmd) => {
      ...
      try {
        ...
        const connection = new Connection(globals.url, 'confirmed');
        const stable = await SolanaStablecoin.load(connection, new PublicKey(globals.mint), authority);
        const sig = await stable.oracle.transferAuthority(new PublicKey(newAuthority));
        spinner.succeed(`Oracle authority transfer initiated → ${newAuthority}`);
        success('The new authority must call "sss-token oracle accept-authority" to complete');
        printTxLink(sig);
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        spinner.succeed('Oracle authority transfer accepted — you are now the oracle authority');
        printTxLink(sig);
      } catch (err: any) {
