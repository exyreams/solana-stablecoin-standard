...
export function privacyCommand(): Command {
  ...
  cmd
    ...
    .description('Approve a token account for confidential transfers (master authority only)')
    .action(async (tokenAccount, opts, cmd) => {
      const globals = cmd.parent!.parent!.opts();
      const spinner = ora('Approving account for confidential transfers...').start();
      try {
        ...
        const sig = await stable.privacy.approveAccount(new PublicKey(tokenAccount));
        spinner.succeed(`Account approved for confidential transfers: ${tokenAccount}`);
        printTxLink(sig);
      } catch (err: any) {
  ...
  cmd
    ...
    .action(async (tokenAccount, opts, cmd) => {
      ...
      try {
        ...
        const connection = new Connection(globals.url, 'confirmed');
        const stable = await SolanaStablecoin.load(connection, new PublicKey(globals.mint), authority);
        const sig = await stable.privacy.enableCredits(new PublicKey(tokenAccount));
  ...
  cmd
    ...
    .description('Disable receiving confidential transfers (account owner only)')
    .action(async (tokenAccount, opts, cmd) => {
      const globals = cmd.parent!.parent!.opts();
      ...
      try {
        ...
        const connection = new Connection(globals.url, 'confirmed');
        const stable = await SolanaStablecoin.load(connection, new PublicKey(globals.mint), authority);
        const sig = await stable.privacy.disableCredits(new PublicKey(tokenAccount));
