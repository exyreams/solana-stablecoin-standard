...
import ora from 'ora';
import { success, error, printTable } from '../../utils/display';
...
export function auditLogCommand(): Command {
  ...
  cmd
    ...
    .option('--action <type>', 'Filter by memo/log content (mint, burn, freeze, blacklist, seize, pause)')
    ...
    .action(async (opts, cmd) => {
      ...
      try {
        ...
        if (opts.action) {
          ...
          success(`Found ${filtered.length} transaction(s) matching "${opts.action}"`);
