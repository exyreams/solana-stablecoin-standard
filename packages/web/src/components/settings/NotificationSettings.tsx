    ...
    <>
      <div className="bg-(--bg-surface) border border-(--border-mid) p-4 mb-4">
        ...
        {toggles.map((toggle, idx) => (
          <div
            key={idx}
            className={`flex justify-between items-center py-3 ${idx < toggles.length - 1 ? "border-b border-(--border-dim)" : ""
              }`}
          >
            <span
              className={`font-mono text-[11px] ${toggle.enabled ? "text-(--text-main)" : "text-(--text-dim)"
                }`}
            >
              {toggle.label}
            </span>
            <div
              className={`w-8 h-4 border relative cursor-pointer ${toggle.enabled
                  ? "bg-green-400 border-green-400"
                  : "bg-(--bg-input) border-(--border-mid)"
                }`}
            >
              <div
                className={`absolute w-3 h-3 top-[1px] transition-all ${toggle.enabled
                    ? "left-4 bg-(--bg-body)"
