...
export const PresetCard: FC<PresetCardProps> = ({
  ...
  return (
    <div
      onClick={onSelect}
      className={`bg-(--bg-surface) border p-6 relative transition-all cursor-pointer flex flex-col ${dashed
    ...
    >
      ...
      <div className="mb-5">
        ...
        <div className="text-[11px] text-(--text-main) font-mono">{useCase}</div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-6">
        {features.map((feature, idx) => (
          <div
            key={idx}
            className={`font-mono text-[9px] px-2 py-1 border flex justify-between items-center ${feature.enabled
                ? feature.color
      ...
      <div className="mt-auto">
        <button
          className={`w-full px-2.5 py-2.5 font-mono text-[11px] border cursor-pointer ${selected
