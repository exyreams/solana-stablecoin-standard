...
export const StandardCard: FC<StandardCardProps> = ({
  ...
  const boxShadow = variant === "recommended"
    ? { boxShadow: 'inset 0 0 10px var(--accent-glow)' }
    : {};

  return (
    <div className={`bg-(bg-panel) border ${borderColor} p-6 relative`} style={boxShadow}>
      {variant === "recommended" && (
      ...
      <p className="text-(text-dim) text-[13px] mb-4 h-10 leading-tight">{description}</p>
