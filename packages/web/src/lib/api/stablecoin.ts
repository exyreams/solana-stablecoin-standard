...
export const stablecoinApi = {
  create: async (data: CreateStablecoinRequest): Promise<CreateStablecoinResponse> => {
    const response = await api.post<CreateStablecoinResponse>("/create-stablecoin", data);
    return response.data;
  },
  get: async (mintAddress: string): Promise<StablecoinDetails> => {
    const response = await api.get<StablecoinDetails>(`/get-stablecoin/${mintAddress}`);
    return response.data;
  },
  list: async (params?: { limit?: number; offset?: number }): Promise<ListStablecoinsResponse> => {
