      ...
      <main className="col-start-2 row-start-2 overflow-y-auto flex bg-(--bg-body)">
        ...
        <aside className="w-64 bg-(--bg-panel) border-r border-(--border-dim) p-4 flex flex-col gap-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-mono text-[10px] text-(--text-main)">DOCS</span>
          </div>
                  ...
                  {filteredDocs
                    ...
                    .map((doc) => (
                      <button
                        ...
                        className={`text-left px-2 py-1.5 font-mono text-[11px] ${selectedDoc === doc.id
            ...
            {selectedDoc === "overview" && (
              <div className="space-y-6 text-(--text-dim) leading-relaxed">
                <p className="text-[14px]">
                  Solana Stablecoin Standard (SSS) is an open-source SDK and on-chain standards
                  for building stablecoins on Solana, developed by Superteam Brazil.
                </p>
                <div className="bg-(--bg-surface) border border-(--border-mid) p-6">
                  <h2 className="font-mono text-[12px] text-(--text-main) mb-4">STANDARDS</h2>
                  <ul className="space-y-3 text-[13px]">
                    <li>
                      <span className="font-mono text-(--accent-primary)">SSS-1 (Minimal)</span> -
                      Basic stablecoin with mint/freeze/metadata
                    </li>
                    <li>
                      <span className="font-mono text-(--accent-primary)">SSS-2 (Compliant)</span>{" "}
                      - SSS-1 + permanent delegate + transfer hook + blacklist
                    </li>
                    <li>
                      <span className="font-mono text-(--accent-primary)">SSS-3 (Private)</span> -
            ...
            {selectedDoc !== "overview" && (
              <div className="bg-(--bg-surface) border border-(--border-mid) p-8 text-center">
                ...
                <p className="text-[13px] text-(--text-dim)">
                  Full documentation content would be loaded here from markdown files.
                </p>
              </div>
            )}
          </article>
