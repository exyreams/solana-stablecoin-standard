...
const Create: FC = () => {
  ...
  const steps = [
    ...
    { number: 2, label: "BASIC INFO", active: currentStep === 2, completed: currentStep > 2 },
    { number: 3, label: "EXTENSIONS", active: currentStep === 3, completed: currentStep > 3 },
    { number: 4, label: "ROLES", active: currentStep === 4, completed: currentStep > 4 },
    { number: 5, label: "ORACLE", active: currentStep === 5, completed: currentStep > 5 },
    { number: 6, label: "REVIEW", active: currentStep === 6, completed: currentStep > 6 },
  ];
  ...
  const renderStep2 = () => (
    <div className="grid grid-cols-[1fr_380px] gap-12 px-8 pb-24">
      <div className="flex flex-col gap-6">
          ...
          <div className="flex bg-(--bg-input) border border-(--border-mid) w-fit">
            {[6, 8, 9].map((d) => (
              <button
                ...
                className={`px-6 py-2 text-[11px] font-mono ${formData.decimals === d
        ...
        <div className="flex flex-col gap-2">
          <label className="font-mono text-[10px] text-(--text-dim) uppercase tracking-wider">
            Metadata URI <span className="text-(--text-dark) normal-case">(Optional)</span>
          </label>
          <input
            type="text"
      ...
      <div className="sticky top-0">
        ...
        <div className="bg-(--bg-surface) border border-(--border-mid) aspect-square flex flex-col items-center justify-center p-10 relative overflow-hidden">
          ...
          <div className="absolute bottom-6 left-6 right-6 flex justify-between font-mono text-[10px] text-(--text-dark)">
            ...
            <div>
              TYPE: SSS-
              {selectedPreset === "sss1" ? "1" : selectedPreset === "sss2" ? "2" : "3"}
            </div>
          </div>
        </div>
      </div>
            ...
            <div
              onClick={() =>
                setFormData({
                  ...
                  extensions: {
                    ...formData.extensions,
                    [ext.key]: !formData.extensions[ext.key as keyof typeof formData.extensions],
                  },
                })
              }
              className={`w-8 h-4 border relative cursor-pointer ${formData.extensions[ext.key as keyof typeof formData.extensions]
                ? "bg-[rgba(204,163,82,0.2)] border-(--accent-primary)"
                : "bg-(--bg-input) border-(--border-mid)"
                }`}
            >
              <div
                className={`absolute w-2.5 h-2.5 top-px transition-all ${formData.extensions[ext.key as keyof typeof formData.extensions]
  ...
  const renderStep4 = () => (
    <div className="px-8 pb-24 max-w-4xl">
      ...
      {[
        ...
        { key: "blacklister", label: "BLACKLISTER AUTHORITY", required: false, badge: "SSS-2" },
      ].map((role) => (
        <div
          key={role.key}
        ...
        >
          <div className="flex flex-col gap-1">
            ...
            {!role.required && (
              <div className="text-[9px] text-(--text-dark) uppercase">Optional</div>
            )}
          </div>
    ...
    <div className="px-8 pb-24 max-w-4xl">
      <div className="bg-(--bg-surface) border border-(--border-mid) p-4 flex items-center gap-4 mb-8">
        <div
          ...
          className={`w-10 h-5 border relative cursor-pointer ${formData.oracle.enabled
            ? "bg-[rgba(204,163,82,0.2)] border-(--accent-primary)"
            : "bg-(--bg-input) border-(--border-mid)"
            }`}
        >
          <div
            className={`absolute w-3.5 h-3.5 top-px transition-all ${formData.oracle.enabled
      ...
      <div className="mt-10 font-mono text-[10px] text-(--text-dark) p-4 border border-dashed border-(--border-mid) text-center">
        Oracle can be configured after deployment from the Oracle Management page. Skip if not
        needed.
      </div>
    </div>
  );
          ...
          <div>
            ...
            <div className="space-y-2 text-[11px]">
              <div className="grid grid-cols-[140px_1fr] font-mono">
                ...
                <span className="text-(--text-main)">{formData.tokenName || "—"}</span>
              </div>
              <div className="grid grid-cols-[140px_1fr] font-mono">
                <span className="text-(--text-dim)">Symbol</span>
                <span className="text-(--text-main)">{formData.tokenSymbol || "—"}</span>
              </div>
              <div className="grid grid-cols-[140px_1fr] font-mono">
      ...
      <div className="bg-(--bg-surface) border border-(--border-mid) flex flex-col">
        ...
        <div className="p-6">
          ...
          <div className="border-t border-(--border-mid) pt-4 flex justify-between items-baseline">
            <div className="font-mono text-[10px] text-(--text-main)">TOTAL ESTIMATED</div>
            <div className="text-xl text-(--accent-primary) font-mono">0.00791 SOL</div>
          </div>
          <div className="mt-6 p-4 bg-(--bg-input) border border-(--border-dim) flex justify-between items-center">
            <div className="font-mono text-[10px] text-(--text-dark)">WALLET BALANCE</div>
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-(--bg-panel) border border-(--border-dim) p-6 flex flex-col">
            <div className="font-mono text-[12px] text-(--text-main) mb-3">ADD MINTER</div>
          ...
          <div className="bg-(--bg-panel) border border-(--border-dim) p-6 flex flex-col">
            <div className="font-mono text-[12px] text-(--text-main) mb-3">MINT TOKENS</div>
            <div className="text-[11px] text-(--text-dim) leading-relaxed mb-4 flex-1">
          ...
          <div className="bg-(--bg-panel) border border-(--border-dim) p-6 flex flex-col">
            <div className="font-mono text-[12px] text-(--text-main) mb-3">GO TO DASHBOARD</div>
            <div className="text-[11px] text-(--text-dim) leading-relaxed mb-4 flex-1">
